...
class UserController extends Controller {
    ...
    public function allUserList(Request $request, $branch) {
        ...
        if (!empty($searchtext)) {
            $userList = User::where([['branch_id', $branch],['role','Staff']])
            ...
            $userList = User::where([['branch_id', $branch],['role','Staff']])->paginate(20);
        }
    ...
    public function updateUser(Request $request, User $user) {
        ...
        $user->update($data);
        return back()->withSuccess("Informations utilisateur mises à jour avec succès");
    }
